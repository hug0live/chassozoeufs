class HideSpotDefinition {
  ...
  String get label => '$area · $objectLabel';
}
...
final List<String> kAreas = [
  for (final area in {for (final spot in kHideSpots) spot.area}) area,
];

List<HideSpotDefinition> hideSpotsForArea(String area) {
  return kHideSpots.where((spot) => spot.area == area).toList(growable: false);
}

HideSpotDefinition hideSpotById(String id) {
  return kHideSpotById[id] ??
      const HideSpotDefinition(
        id: 'unknown',
        area: 'Inconnu',
        objectLabel: 'cachette inconnue',
        riddle: "Cette cachette n'est plus disponible dans le catalogue.",
        hint: "Demande au maitre du jeu ou il a range cet oeuf.",
      );
}
